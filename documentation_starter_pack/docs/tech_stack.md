...
| UI Components | Custom, Material-UI, Ant Design | **shadcn/ui** (Radix + Tailwind) + **canvas-confetti 1.9.4** | Unstyled, accessible components. Copy-paste, full control. Boilerplate includes 20+ components. canvas-confetti for celebration animations (11KB). | Full customization, no bundle bloat, accessible, delightful UX | Manual updates (not npm package) | Control vs convenience | Risk: Component drift. Mitigation: Lock shadcn version, document customizations | Using Tailwind 4.1 + CVA for variants. Confetti lazy-loaded for progress feedback |
